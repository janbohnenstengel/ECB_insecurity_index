...
all_text_data <- as.data.frame(readRDS("data/all_textual_data_separate.rds"))

combined_text_data <- as.data.frame(readRDS("data/all_text_per_meeting_combined.rds"))
```
...
# Statements
all_text_data$statement_score <- sapply(all_text_data$statement_processed, count_uncertainty_norm,
                                        dictionary = risk_dictionary_stemmed)

# Q&As
all_text_data$qa_score <- sapply(all_text_data$qa_processed, count_uncertainty_norm,
                                 dictionary = risk_dictionary_stemmed)

# Accounts
all_text_data$account_score <- sapply(all_text_data$account_processed, count_uncertainty_norm,
                                      dictionary = risk_dictionary_stemmed)
...
# Combined texts
combined_text_data$dictionary_score <- sapply(combined_text_data$text, count_uncertainty_norm, 
                                              dictionary = risk_dictionary_stemmed) 
```
...
cor(all_text_data$statement_score, all_text_data$qa_score)

# Accounts only Plot
accounts_score_plot <- all_text_data |>
    ...
    theme_minimal()
accounts_score_plot
ggsave("plots/account_score_over_time.png", accounts_score_plot, height = 5, width = 8,
       bg = "white", dpi = 320)

# Statements only Plot
statement_score_plot <- all_text_data |>
...
ggsave("plots/statement_score_over_time.png", statement_score_plot, height = 5, width = 8,
       bg = "white", dpi = 320)

# Q&As only Plot
qa_score_plot <- all_text_data |>
    ...
    theme_minimal()
qa_score_plot
ggsave("plots/qa_score_over_time.png", qa_score_plot, height = 5, width = 8,
       bg = "white", dpi = 320)
...
pasted_score_plot <- combined_text_data |>
    ...
    theme_minimal()
pasted_score_plot
ggsave("plots/pasted_score_over_time.png", pasted_score_plot, height = 5, width = 8,
       bg = "white", dpi = 320)
```
